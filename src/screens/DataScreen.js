...
export default function DataScreen() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      addLog(`File created: ${formatBytes(info.size)}`, 'success');
      ...
      await Sharing.shareAsync(fileUri, {
        ...
        dialogTitle: 'Save your TimeTracker backup',
        UTI:        'public.json',
      });

      setLastBackupInfo({ exportedAt: Date.now(), size: info.size });
      addLog(`Backup shared: ${goals.length} goals, ${sessions.length} sessions`, 'success');
    } catch (err) {
  ...
  async function handleImport() {
    try {
      ...
      if (!payload._meta || payload._meta.appName !== 'TimeTracker') {
        throw new Error('File was not created by TimeTracker. Import aborted.');
      }
      if (!Array.isArray(payload.goals)) {
        throw new Error('Backup is missing the goals array.');
      }
      if (!Array.isArray(payload.sessions)) {
        throw new Error('Backup is missing the sessions array.');
      }
      ...
      addLog(`Found ${goalCount} goals, ${sessionCount} sessions (backup from ${backupDate})`);

      // ── Confirm with user ─────────────────────────────────────────────────
      await new Promise((resolve, reject) => {
        Alert.alert(
          'Restore Backup?',
          `This will REPLACE all your current data with:\n\n• ${goalCount} goals\n• ${sessionCount} sessions\n• Exported on ${backupDate}\n\nThis cannot be undone.`,
          [
      ...
      // ── Write to storage ──────────────────────────────────────────────────
      addLog('Writing goals…');
      await saveGoals(payload.goals);

      addLog('Writing sessions…');
      await saveSessions(payload.sessions);

      addLog('Writing active session…');
      await saveActiveSession(payload.activeSession ?? null);

      await loadStats();
      addLog(`✓ Restore complete — ${goalCount} goals, ${sessionCount} sessions`, 'success');
      Alert.alert('Restored!', `Successfully imported ${goalCount} goals and ${sessionCount} sessions.`);
    } catch (err) {
  ...
  async function handleReset() {
    Alert.alert(
      '⚠️ Delete All Data',
      'This will permanently delete all your goals, sessions, and settings. You cannot undo this.',
      [
      ...
      >
        ...
        {stats && (
          <View style={s.summaryCard}>
            ...
            <View style={s.summaryGrid}>
              ...
              <StatBlock
                label="Total Time"
                value={fmtHours(stats.totalSecs)}
              />
            </View>
            {stats.active && (
              <View style={s.activeBadge}>
                ...
                <Text style={s.activeBadgeText}>
                  Timer running — will be included in backup
                </Text>
              </View>
            )}
          </View>
          ...
          <View style={s.actionCard}>
            <View style={s.actionInfo}>
              ...
              <Text style={s.actionDesc}>
                Saves all goals + sessions as a{' '}
                <Text style={s.mono}>.json</Text> file.{'\n'}
                Share it to Google Drive, email, or any app.
              </Text>
              {lastBackupInfo && (
                <Text style={s.actionMeta}>
                  Last export: {formatDate(lastBackupInfo.exportedAt)} · {formatBytes(lastBackupInfo.size)}
                </Text>
              )}
            </View>
            <TouchableOpacity
            ...
            >
              {exporting
                ? <ActivityIndicator color="#0a1a0a" size="small" />
                : <Text style={s.btnExportText}>↑  Export</Text>
              }
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={s.section}>
          ...
          <View style={s.actionCard}>
            <View style={s.actionInfo}>
              ...
              <Text style={s.actionDesc}>
                Pick a <Text style={s.mono}>.json</Text> backup file.{'\n'}
                Your current data will be replaced.
              </Text>
            </View>
            <TouchableOpacity
              style={[s.btn, s.btnImport, importing && s.btnDisabled]}
            ...
            >
              {importing
                ...
                : <Text style={s.btnImportText}>↓  Import</Text>
              }
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={s.section}>
          ...
          <View style={s.guideCard}>
            {[
              { n: '1', text: 'Tap Export Backup on your old phone.' },
              { n: '2', text: 'Share the .json file to Google Drive, WhatsApp, email, or any app.' },
              { n: '3', text: 'Open the file on your new phone.' },
              { n: '4', text: 'Tap Import Backup here and pick the file.' },
            ].map(step => (
        ...
        <View style={s.section}>
          ...
          <View style={[s.actionCard, s.dangerCard]}>
            <View style={s.actionInfo}>
              ...
              <Text style={s.actionDesc}>
                Permanently wipes all goals and sessions.{'\n'}
        ...
        <Text style={s.footnote}>
          Backups are stored locally — nothing is uploaded to any server.
        </Text>
      </ScrollView>
    </SafeAreaView>
  );
